...
This v1.7.0 package makes Flint a first-class **MCP-gated** Scout capability.
...
1. Run the published `Install-ActMcpProfile.ps1 -Profile flint` to inspect its
   exact change. It targets the reviewed Scout user configuration and creates
   no file or configuration change in preview mode.
...
`Uninstall-ActMcpProfile.ps1 -Profile flint -Apply` script. It backs up the
configuration and refuses to remove a differently configured entry. Restart
Scout and confirm the server is disconnected. Do not delete a runtime or modify
host configuration as a substitute for a reviewed rollback plan.
