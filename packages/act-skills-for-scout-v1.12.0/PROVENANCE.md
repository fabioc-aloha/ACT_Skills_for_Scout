# v1.4.0 Provenance and Adaptation
...
calls, or source-repository assumptions. The package-only non-mutating Flint
readiness preview is outside all direct skill folders and is not a skill
payload. Source references were not copied unless they were already part of
the retained v0.4.0 package contract.
...
| `act-flint-readiness` | `Alex_ACT_Illustrator_Plugin\docs\flint-mcp-0.5.1-capability-matrix.md`; `.github\skills\setup-illustrator-runtime\SKILL.md`; `.github\skills\setup-illustrator-runtime\scripts\runtime-launcher.mjs` | Adapts the reviewed `flint-chart-mcp@0.5.1` capability names into host-observed, approval-gated readiness guidance without carrying over configuration paths, runtime state, installation behavior, or source scripts. |
| `act-flint-chart` | `Alex_ACT_Illustrator_Plugin\.github\skills\flint-chart\SKILL.md`; `docs\flint-mcp-0.5.1-capability-matrix.md` | Curates MCP-gated chart framing, version-matched grammar authority, backend limits, untrusted-reference handling, semantic-versus-theme separation, and visual verification without copying Flint grammar or tool invocation syntax. |
| `act-flint-theme` | `Alex_ACT_Illustrator_Plugin\.github\skills\flint-theme\SKILL.md`; `docs\flint-mcp-0.5.1-capability-matrix.md` | Curates MCP-gated theme workflow, ThemeSpec limits, untrusted-reference handling, semantic-versus-presentation separation, and corpus visual verification without copying ThemeSpec grammar. |
...
All plugin, runtime, platform, browser, source-script, and source-ownership
artifacts are excluded. The three Flint skills retain only the reviewed
`flint-chart-mcp@0.5.1` capability names and limitations necessary to gate use
on actual host exposure; they do not carry Flint grammar, configuration, or
installation behavior. In particular, no content was sourced from
`Alex_ACT_Steward\_github_backup` or any archive.
