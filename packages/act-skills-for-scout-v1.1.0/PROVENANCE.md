# v1.0.0 Provenance and Adaptation
...
| `act-skill-inventory-report` | User-specified Scout inventory reporting contract | Uses the host API as authoritative and only enriches host-reported ACT rows with the optional local library manifest. |
...
source-ownership artifacts are also excluded. In particular, no content was
