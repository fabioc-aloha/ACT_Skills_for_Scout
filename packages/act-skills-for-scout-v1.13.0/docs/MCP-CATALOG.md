...
| `powerbi-remote-pilot` | Controlled pilot | Official hosted Power BI Remote MCP at `https://api.fabric.microsoft.com/v1/mcp/powerbi`, restricted to semantic-model schema, report summary, and report metadata tools; canary: `GetReportMetadata` against a report the signed-in user can access. Interactive OAuth is required; the catalog contains no token or OAuth alias. |
...
`youtube-mcp-tools`, and `powerbi-remote-pilot`. The Power BI profile registers
only the endpoint and reviewed tool allow-list; sign in interactively when Scout
prompts. It registers `fabric-rti-ro` only after validating the environment file
above. It does not install any managed, deferred, or discovery-only catalog
record.
