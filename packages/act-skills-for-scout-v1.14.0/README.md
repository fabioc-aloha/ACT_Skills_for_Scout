# ACT Skills for Scout v1.14.0

Prepared Scout-native package for user-global Scout discovery through the
OneDrive-backed ACT skill library. Publish it to the source user's library with
the documented installer. The v1.13.0 predecessor passed full safe,
read-only acceptance coverage; see the source delivery documentation for
...
`v1.14.0` adds a non-mutating MCP registration verifier and removes Power BI
Remote Pilot from automatic registration because the host exposed tools beyond
its declared metadata allow-list. `v1.13.0` added the initial Power BI Remote
Pilot profile. `v1.12.0` automatically registers the five validated MCP
profiles during every applied ACT skill installation. Fabric RTI remains
conditional on a validated non-production Kusto allow-list.
`v1.11.0` makes `loadCopilotCliSkills` a required, backup-protected part of
every applied ACT skill installation so the user-global junctions appear in
Scout's Skills UI.
`v1.10.0` corrects the `fabric-docs-ro` runtime contract for Windows ARM64:
the pinned .NET tool installs as `fabmcp.cmd` and exposes its documentation
surface through the hierarchical `docs` tool.
`v1.9.0` repairs the controlled-pilot `youtube-mcp-tools` profile. It now
builds the reviewed source commit in a dedicated local runtime before
registration rather than relying on an unavailable npm artifact.
