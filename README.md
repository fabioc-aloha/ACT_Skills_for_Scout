...
present. Power BI Remote Pilot is catalogued but not automatically registered
until Scout can enforce its remote tool allow-list.
...
Flint, Fabric docs, Azure DevOps read-only, Azure Kusto read-only, and YouTube.
Use the profile installer only to preview, remove, or explicitly
manage a profile:
...
on its non-production allow-list. Power BI Remote Pilot remains catalogued but
not automatically registered until its host enforcement boundary is verified.
